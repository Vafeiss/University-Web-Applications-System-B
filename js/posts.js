...
const BASE_URL = "/student/backend/controllers/PostController.php";
const CAT_URL = "/student/backend/controllers/CategoryController.php";
const FOLLOW_URL = "/student/backend/controllers/FollowController.php";
const NOTIFICATION_URL = "/student/backend/controllers/NotificationController.php";
const SEARCH_URL = "/student/backend/controllers/search_controllers.php";
...
async function loadPendingPostsFeed(options = {}) {
    ...
    try {
        ...
        // Εμφάνιση flash message (αν υπάρχει από νέα δημοσίευση) με το standard inline-notice
        const flash = consumePendingPostFlash();
        if (flash && flash.message) {
            showInlineNotice(flash.message, flash.type || "success");

            // Πολλαπλά refresh attempts: το server-side processing μπορεί να πάρει
            // διαφορετικό χρόνο ανάλογα με το μέγεθος των αρχείων και τους admins.
            // Ξανατραβάμε τα posts στα 1.5s, 3.5s, 6s για σιγουριά.
            [1500, 3500, 6000].forEach((delay) => {
                setTimeout(() => {
                    loadPendingPostsFeed({ silent: true, filters });
                }, delay);
            });
        }
    } catch (error) {
...
function consumePendingPostFlash() {
    try {
        const raw = sessionStorage.getItem("pendingPostFlash");
        if (!raw) return null;
        sessionStorage.removeItem("pendingPostFlash");
        const parsed = JSON.parse(raw);
        // Αγνόησε τα flash messages μεγαλύτερα από 30 δευτερόλεπτα (stale)
